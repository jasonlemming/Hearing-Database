...
@hearings_bp.route('/hearings/v2')
def hearings_v2():
    """Browse hearings - v2 minimal design"""
    try:
        search = request.args.get('search', '')
        chamber = request.args.get('chamber', '')
        committee_id = request.args.get('committee', '')
        ...
        if not date_from and not date_to:
            ...
            # Check if it's this month
                ...
                if date_from == month_start.strftime('%Y-%m-%d') and date_to == month_end.strftime('%Y-%m-%d'):
                    active_filter = 'this-month'
                # If it doesn't match any preset, set to None (no tab highlighted)
...
@hearings_bp.route('/hearing/<int:hearing_id>/v2')
def hearing_detail_v2(hearing_id):
    """Hearing detail page - v2 minimal design"""
    try:
        with db.transaction() as conn:
            # Get hearing info
            ...
            # Get associated committees with parent committee info
            cursor = conn.execute('''
                SELECT c.committee_id, c.name, c.system_code, hc.is_primary,
                       parent.committee_id as parent_id, parent.name as parent_name
                FROM committees c
                JOIN hearing_committees hc ON c.committee_id = hc.committee_id
                LEFT JOIN committees parent ON c.parent_committee_id = parent.committee_id
                WHERE hc.hearing_id = ?
