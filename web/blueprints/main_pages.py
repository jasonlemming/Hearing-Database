...
@main_pages_bp.route('/members/v2')
def members_v2():
    """Browse members - v2 minimal design"""
    try:
        party = request.args.get('party', '')
        state = request.args.get('state', '')
        chamber = request.args.get('chamber', '')
        ...
        return render_template('members_v2.html',
                             members=members_data,
                             parties=parties,
                             states=states,
                             chambers=chambers,
...
@main_pages_bp.route('/member/v2/<int:member_id>')
def member_detail_v2(member_id):
    """Member detail page - v2 minimal design"""
    try:
        with db.transaction() as conn:
            # Get member details
            cursor = conn.execute('''
...
@main_pages_bp.route('/witnesses/v2')
def witnesses_v2():
    """Browse witnesses - v2 minimal design"""
    try:
        search = request.args.get('search', '')
        witness_type = request.args.get('type', '')
        ...
        return render_template('witnesses_v2.html',
                             witnesses=witnesses_data,
                             witness_types=witness_types,
                             search=search,
                             selected_type=witness_type,
...
@main_pages_bp.route('/witness/v2/<int:witness_id>')
def witness_detail_v2(witness_id):
    """Witness detail page - v2 minimal design"""
    try:
        with db.transaction() as conn:
            # Get witness details
            cursor = conn.execute('''
